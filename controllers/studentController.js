...
exports.createStudent = async (req, res) => {
  try {
    const { fullName, email, password, admissionNumber, classId, dateOfBirth, gender, address, parentIds } = req.body;
    ...
    const student = await Student.create({
      user: user._id, admissionNumber, class: classId || undefined,
      dateOfBirth, gender, address, parents: parentIds || [],
    });
    ...
    if (parentIds && parentIds.length) {
      await Parent.updateMany({ _id: { $in: parentIds } }, { $addToSet: { children: student._id } });
    }
    res.status(201).json({ student, user: { id: user._id, fullName, email, role: user.role } });
  } catch (err) {
...
exports.getStudentById = async (req, res) => {
  try {
    const student = await Student.findById(req.params.id)
      .populate('user', 'fullName email photoUrl status')
      .populate('class', 'name')
      .populate({ path: 'parents', populate: { path: 'user', select: 'fullName email' } });
...
exports.updateStudent = async (req, res) => {
  try {
    const { fullName, classId, dateOfBirth, gender, address, status, parentIds } = req.body;
    const student = await Student.findById(req.params.id);
    if (!student) return res.status(404).json({ message: 'Student not found' });
